-- CA-807 (2/4): invite_project_members + respond_to_invitation RPCs.
-- All member mutations go through SECURITY DEFINER RPCs; the invariants
-- (invite_member permission, level rule, registered-vs-unregistered branching)
-- are checked against the DATABASE, never the JWT, so a stale token can never
-- authorize a write (CA-784 "JWT staleness strategy").

-- The registered-user lookup below matches emails case-insensitively; the
-- UNIQUE index on users.email is case-sensitive and cannot serve it.
...
-- =============================================================================
-- Internal helper: process an invite batch for a project on behalf of an
-- inviter. Checks invite_member + the level rule against the database, then
-- branches per email: registered -> project_members row + notification,
-- unregistered -> project_invitations upsert. Returns per-email outcomes:
-- [{"email": ..., "result": "invited" | "already_member" | "pending_signup"}].
-- Not exposed to API roles; called by invite_project_members (and by
-- create_project_with_members in CA-808).
-- =============================================================================
...
-- =============================================================================
-- invite_project_members(project_id, invites): batch-invite existing users and
-- unregistered emails. Returns per-email outcomes. Any invalid entry (unknown
-- role, level-rule violation) aborts the whole batch.
-- =============================================================================
...
-- =============================================================================
-- respond_to_invitation(project_id, accept): the invited user accepts or
-- declines their own pending membership; a matching project_invitations row
-- (when the membership originated from an unregistered-email invite) is
-- marked accordingly.
-- =============================================================================
